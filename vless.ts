...
export class VlessPayload {
    ...
    constructor(vlessBuffer: Uint8Array) {
        ...
        this.userUUID = toUUIDWithoutDash(new Uint8Array(vlessBuffer.slice(1, 17)));
        ...
        if (command === 1) {
            ...
            this.hasError = true;
            this.message = `command ${command} is not support, command 01-tcp,02-udp,03-mux`;
        ...
        switch (addressType) {
            ...
            case 3: {         // IPv6 
                addressLength = 16;
                const ipv6bytes: Uint8Array = vlessBuffer.slice(addressValueIndex, addressValueIndex + addressLength);
        ...
        if (!addressValue) {
            this.hasError = true;
            this.message = `addressValue is empty, addressType is ${addressType}`;
            return;
        }
...
export class Session {
    ...
    public start() {
        const session = this;
        session.webSocketReadStream.pipeTo(new WritableStream<Uint8Array>({
            async write(chunk: Uint8Array, controller) {
                ...
                if (hasError) {
                    controller.error('parse VLESS protocol data failed:' + message);
                    return;
                }
                ...
                if (remoteProtocol === 'udp' && remotePort === 53) {
                    session.outboundDns(firstChunkPayload).catch((error) => {
                        controller.error('handleDnsOutBound has error:' + error.message);
                    });
                } else if (remoteProtocol === 'tcp' && remotePort === 25) {
                    controller.error('cloudflare does not support tcp on port 25');
                } else if (remoteProtocol === 'tcp') {
                    //do not add await here, because we need to handle tcp out bound in parallel
                    session.outboundTcp(firstChunkPayload).catch((error) => {
                        controller.error('handleTcpOutBoundInner has error:' + error.message);
                    });
                } else if (remoteProtocol === 'udp') {
                    try {
                        session.outboundUdp(firstChunkPayload)
                    } catch (error) {
                        controller.error('handleUdpOutBound has error:' + error.message);
                    }

                } else {
                    console.error('udp not support initial connect' + remoteProtocol + remotePort);
            ...
            abort(reason) {
                console.error(`readableWebSocketStream is abort`, JSON.stringify(reason));
            }
        })).catch((err) => {
            console.error('wsReadStream pipeTo error', err);
        }).finally(() => {
            //record traffic
            if (session.trafficFn) {
                session.trafficFn(session.userUuidWithoutDash, session.traffic)
            }
        });
    }

    public get isOutboundSockReady(): boolean {
        return this.outboundTcpSock !== undefined || this.outboundDnsSock !== undefined;
    }
        ...
        return new ReadableStream({
            start(controller) {
                ...
                webSocket.onclose = (event: CloseEvent) => {
                    console.info('webSocketServer is close', event.code, event.reason);
                    controller.close();
                }
    ...
    private async tcpSockPipeToWebSocket(): Promise<boolean> {
        ...
        const webSocketWriter = new WritableStream({
            ...
            async write(chunk: ArrayBufferLike, controller) {
                try {
                    await session.writeToWebsocket(chunk);
                    isRemoteSocketHasIncomingData = true;
                } catch (error:unknown) {
                    console.error('webSocketWriter!.writable has error',  error);
                }
            },
            close() {
                console.info(`webSocketWriter!.writable is close with hasIncomingData is ${isRemoteSocketHasIncomingData}`);
                session.outboundTcpSock && session.outboundTcpSock.close();
            },
            abort(reason) {
                console.error(`webSocketWriter!.writable abort`, reason);
            }
        });

        await session.outboundTcpSock.readable.pipeTo(webSocketWriter).catch((error) => {
            console.error(`tcpSocket.readable.pipeTo has exception `, error.stack || error);
            session.closeWebSocket();
        });
        if (!isRemoteSocketHasIncomingData) {
            console.log('tcpSockPipeToWebSocket has no incoming data, close ws');
        }
        return isRemoteSocketHasIncomingData;
    }

    private async writeToOutbound(chunk: ArrayBuffer) {
        const tcpSocket: Deno.TcpConn | TransformStream | undefined = this.outboundTcpSock || this.outboundDnsSock;
        if (tcpSocket) {
    ...
    private async writeToWebsocket(chunk: ArrayBuffer) {
        ...
        if (this.hasSentWsVersionHeader) {
            ...
            this.webSocket.send(await new Blob([vlessResponseHeader, chunk]).arrayBuffer());
    ...
    private async outboundDns(initialData: ArrayBuffer) {
        ...
        const { writeToWebsocket, writeToOutbound } = this;
        transformStream.readable.pipeTo(new WritableStream({
            async write(chunk) {
                ...
                const udpChunk: ArrayBuffer = await new Blob([udpSizeBuffer, dnsQueryResult]).arrayBuffer();
                await writeToWebsocket(udpChunk);
            }
        })).catch((error) => {
            console.error('dns udp has error' + error);
        });
        await writeToOutbound(initialData);
    }

    private outboundUdp(initialPayload: ArrayBuffer) {
        throw new Error('udp not support initial connect;todo when cloudflare supports udp');
    }
